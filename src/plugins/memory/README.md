...
跨会话记忆插件：同 workspace 的 compaction checkpoint 自动收割进本地 sqlite，新会话开工时按持久节过滤、按预算裁剪后注入 system prompt；`memory_write` / `memory_list` / `memory_forget` 三个工具支持主动沉淀与清理。零额外 LLM 调用、零 compaction 干预——纯事件观察者。
...
- **动态注入。** system prompt 每次组装现读 store，刚写入的记忆下一步立即生效；记忆变更后发出 `system-prompt/change`。
...
细节：宿主半边 `src/index.ts`（`name: dsh-memory`）只注入 `systemPrompt` 与 `tools` 两个服务；收割全部 try/catch 只记 warning 日志，绝不向宿主事件流抛错。存储与纯逻辑见 `store.ts` / `pure.ts`，测试覆盖见 `pure.spec.ts` / `store.spec.ts` / `index.spec.ts`。