...
- **tab 说明。** 会话头部 `conversation.view` 第三个 tab：`id: 'memory'`、order 20（Chat 0 / Trajectory 10 之后）、label "Memory"。tab 跟随当前打开的会话，不做子代理特判（子代理会话自然显示它自己的注入块）。内容以 **markdown 渲染**：走宿主冻结原语 `MarkdownText`（模块加载器共享，不加新依赖、不进 bundle），`##` 标题与 bullet 列表正常排版，代码 fence 走 shiki 高亮；`<project-memory>` 这类 XML 风格标签保持字面量文本（原生 HTML 关闭，不会被当成标签解析）。内容字节不改——渲染只是呈现层，落在卡片底面上（结构化卡片仍留给 v2）；长行由 markdown 正文自然换行，无需横向滚动，也不再有等宽 `white-space: pre-wrap` 姿态。无记忆时显示空态文案「No memories for this session yet」。
