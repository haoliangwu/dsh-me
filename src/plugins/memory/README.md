...
- **动机。** 注入的 memory block 藏在会话上下文里，用户看不见模型到底看到了什么；排查「记忆为什么没生效 / 注入了什么」只能翻日志或用 `memory_list` 看原料。Memory tab 把**注入结果**渲染出来——内容字节与注入完全一致，模型视角，所见即模型所得。
- **tab 说明。** 会话头部 `conversation.view` 第三个 tab：`id: 'memory'`、order 20（Chat 0 / Trajectory 10 之后）、label "Memory"。tab 跟随当前打开的会话，不做子代理特判（子代理会话自然显示它自己的注入块）。内容以 **markdown 渲染**：走宿主冻结原语 `MarkdownText`（模块加载器共享，不加新依赖、不进 bundle），`##` 标题与 bullet 列表正常排版，代码 fence 走 shiki 高亮；wire 格式的 XML 标签行在呈现层隐藏（wire 字节不动，纯视图变换）：纯包装/闭合标签行（`<project-memory>`、`</note>`、`</checkpoint>`）整行去掉，note 行剥掉标签只留内容，`<checkpoint …>` 开标签转成 `> Checkpoint · 日期 · 短会话 id` 的次要样式 caption（blockquote，宿主 caption 色左线，兼作条目之间的分隔线）；未知的纯标签行仍包进独立代码 span，防止开启 CommonMark HTML block 吞掉后续内容。内容字节不改——渲染只是呈现层，落在卡片底面上（结构化卡片仍留给 v2）；长行由 markdown 正文自然换行，无需横向滚动，也不再有等宽 `white-space: pre-wrap` 姿态。无记忆时显示空态文案「No memories for this session yet」。
- **数据通道。** node 半边挂 webServer 前缀路由 `/dsh-memory`，手刻 Connection-RPC envelope（peak-rate/undo 同款 serveChannel 模式；profile 插件树用不了 `connection.rpc.handle`），endpoint `block` 入参 `{sessionId, cwd}`，现算 `assembleMemoryBlock(store.listActive(cwdToWorkspaceKey(cwd), sessionId), budget, sessionId)`——与 `agent/pre-step` 注入**同一套纯函数**，输出块字节一致。client 半边 `ctx.connection.rpc.call('/dsh-memory', 'block', {sessionId, cwd})`；sessionId 来自 tab 注入入参，cwd 来自 `ctx.sessions.list` 快照。入参缺省（空 sessionId/cwd）返回 `{block: ''}`；未知 endpoint 返回标准 RPC error shape。
- **overlay 姿态。** 视图根节点带 `data-conversation-composer-overlay`（TrajectoryView 同款官方 opt-out）：宿主隐藏宽度拖柄、`.scrollBody` 变纯裁剪盒、composer 绝对定位——tab 激活期间宽度从 Chat / Trajectory tab 调整；视图自身 `height: 100%` + `overflow: auto` 全出血自滚动。底部 `padding: 16px 24px calc(var(--dsh-composer-height, 152px) + 16px)` 预留浮动 composer 高度（宿主 ConversationRoot 的 seat 观察器 live 发布 `--dsh-composer-height`，+16px 呼吸位；Trajectory / Chat 同款 calc），块尾不再被输入条压住。滚动内容列（toolbar + 卡片）遵循宿主内容列宽度：`max-width: var(--dsh-chat-content-width); margin-inline: auto`（ChatView `.column` 同款契约）——composer 输入条顶部 36px 渐隐遮罩仅在输入卡下方不透明，内容列居中后两侧沟槽只剩 `bg-base`，滚动文字不会从遮罩半透明带里穿过输入卡两侧。
