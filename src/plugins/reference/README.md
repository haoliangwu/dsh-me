...
把 [OpenCode references](https://opencode.ai/v2/docs/references/) 的能力带进 dsh：命名的引用表（alias → 本地目录或 Git 仓库 + 描述 + 自动包含），设置页可视化管理，Git 引用自动物化（clone/刷新）到缓存目录，@ 菜单按别名挂载，自动包含的引用广告进系统提示（`<available_references>` XML 块）— agent 知道有哪些资料、在哪、何时该查。dsh 读操作本无路径边界，无需任何权限机制。
...
- **90+ 个 vitest 用例**守护纯决策核（XOR 校验、refresh 决策矩阵、git 命令组装、广告组装、@ 候选列出、提及序列化、选择器命令三平台分支）

## 使用

设置页左侧导航 **References**：新增/编辑（类型切换 Local / Git，字段随形态）、两步确认删除。保存的引用立即进 @ 菜单；自动包含（默认开）的引用同时广告进系统提示。

- **Local 形态**：path + description + 自动包含开关。path 支持绝对路径与 `~/` 开头，相对路径拒绝；指向不存在目录可保存但行内标 ⚠。「Choose folder」弹**系统原生**目录选择框（macOS osascript / Linux zenity→kdialog / Windows PowerShell FolderBrowserDialog），返回真实绝对路径。
- **Git 形态**：repository URL（必填，`file://` 拒绝）+ branch（可选）+ description + 自动包含开关 + 「每次刷新」开关。保存后插件后台 `clone --depth 1`（branch 未填 = 默认分支）到 `<cacheDir>/<alias>`；行内展示 repository（+ branch / 每次刷新标记），不做 ⚠ 路径探测。
...
无 description 的引用若自动包含仍广告（只列 name/path，省略 `<description>` 元素）；空表、或所有引用都关了自动包含时，不注入任何内容。**自动包含（autoInclude，默认开）只控制系统提示广告**：开 = 广告进 `<available_references>`，agent 被告知；关 = 该引用从广告中消失，但 **仍在 @ 菜单**，仍可手动 @ 挂载（agent 不会被告知，只能等用户主动 @）。设置页里该开关以正向语义呈现为「自动包含」：打开 = 自动进广告，关闭 = 仅手动 @ 挂载。兼容旧数据：settings.yaml 里的旧 `hidden: true` 读入时迁移为 `autoInclude: false`（旧 hidden 用户明确不想被自动提及），旧 false/缺省 → 开；写入只用新字段。
...
| 挂载 | `InputTriggerSource`（trigger `@`，name `dsh-reference`），**所有条目**都是候选（autoInclude 只裁剪广告，不裁剪菜单），codec 落纯文本提及 |
