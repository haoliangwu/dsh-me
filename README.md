...
| [memory](src/plugins/memory/README.md) | `dsh-memory` | 跨会话记忆：compaction summary 自动收割进 sqlite、按持久节过滤后作为持久 context 行注入（`agent/pre-step`，plugin source + digest，系统 prompt 字节恒定不杀前缀缓存），`memory_write`/`memory_list`/`memory_forget` 主动记录；会话头部只读 Memory tab 逐字查看注入块（模型视角，手动刷新） |
